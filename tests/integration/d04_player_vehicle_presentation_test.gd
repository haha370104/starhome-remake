...
func _run() -> void:
	...
	_expect(player.combat_presenter.get_child_count() == 3, "战车占位外观只有阴影、底盘和主装置")
	_expect(player.combat_presenter._layers[&"primary_weapon"].visible, "默认应显示能量炮层")
	_expect(not player.combat_presenter._layers.has(&"missile_weapon"), "副武器不得占据战车主炮位置")
	hall.hud.set_tactical_action("missile")
	hall.hud.select_action("missile")
	_expect(player.combat_presenter._layers[&"primary_weapon"].visible, "选择导弹后主炮仍保持原样")
