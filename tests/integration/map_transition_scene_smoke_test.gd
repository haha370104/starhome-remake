...
func _run() -> void:
	...
	_expect(hall.active_world_controller.definition.map_id == &"yian_harbor_hall_floor_1", "测试必须从荣耀版大厅开始")
	...
	hall.interactions.call("move_to", Vector2(900, 1300))
	_expect(
	...
	hall.local_player_controller.path_points = PackedVector2Array([held_position, held_position + Vector2(100, 0)])
	hall.local_player_controller.path_index = 1
	hall.local_player_controller.active_movement_input_sequence = 7
	hall.map_travel.call("hold_old_map_for_authoritative_join",
		&"yian_harbor_city",
		"yian_harbor_city.instance.review",
		Vector2(1399, 954),
	)
	_expect(hall.local_player_controller.path_points.is_empty(), "异步权威切图必须立即终止旧地图路径")
	_expect(hall.local_player_controller.active_movement_input_sequence == 0, "异步权威切图必须停止记录旧地图预测输入")
	...
	_expect(hall.active_world_controller.definition.map_id == &"yian_harbor_city", "大厅出口必须进入真实 City1Svr 业务图")
	_expect(hall.world_view.player.position == Vector2(1399, 954), "城市入口0必须采用配置化权威落点")
	_expect(hall.active_world_controller.navigation.grid_size == Vector2i(71, 560), "城市必须切换到自己的荣耀导航")
	_expect(hall.active_world_controller.scene_nodes.size() == 888, "城市必须提交当前荣耀清单中的全部语义遮挡层")
	_expect(hall.active_world_controller.npc_instances.is_empty(), "大厅 NPC 不得泄漏到城市")
	...
	_expect(hall.active_world_controller.definition.map_id == &"d04_field_zone", "城市西北门必须进入 D04")
	_expect(hall.world_view.player.position == Vector2(1290, 2562), "D04入口1必须采用配置化权威落点")
	_expect(hall.active_world_controller.navigation.grid_size == Vector2i(101, 800), "D04必须切换到自己的荣耀导航")
	_expect(hall.active_world_controller.scene_nodes.size() == 117, "D04必须提交官网惰性资源恢复后的荣耀语义遮挡层")
	...
	_expect(hall.active_world_controller.definition.map_id == &"buli_c04_field_zone", "D04 西侧出口必须进入布里 C04")
	_expect(hall.world_view.player.position == Vector2(4687, 2525), "D04→C04 必须落在 C04 指回 D04 的边缘出口")
	_expect(hall.active_world_controller.navigation.grid_size == Vector2i(101, 800), "C04 必须提交自己的荣耀导航")
	_expect(not hall.active_world_controller.scene_nodes.is_empty(), "C04 必须提交已打包的场景表现")
	...
	_expect(hall.active_world_controller.definition.map_id == &"buli_c03_field_zone", "C04 北侧出口必须进入布里 C03")
	_expect(hall.world_view.player.position == Vector2(2426, 4565), "C04→C03 必须落在 C03 正下方反向出口")
	_expect(not hall.active_world_controller.scene_nodes.is_empty(), "C03 必须提交已打包的场景表现")
	...
	hall.interactions.call("move_to", Vector2(1200, 2500))
...
func _wait_for_map(hall: Node2D, expected_map_id: StringName) -> void:
	for _frame in range(600):
		if hall.active_world_controller.definition.map_id == expected_map_id:
			return
		await process_frame
	_fail(
...
func _assert_current_map_monsters(hall: Node2D) -> void:
	...
	var instance: AuthoritativeMapInstance = server.map_registry.instance_by_map_id(hall.active_world_controller.definition.map_id)
