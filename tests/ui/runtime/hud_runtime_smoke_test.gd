...
func _assert_1280_layout(hud: CanvasLayer) -> void:
	_expect(hud.top_menu.size == Vector2(156, 56), "顶部菜单必须保持 156×56 紧凑双排")
	_expect(hud.top_menu.position == Vector2(999, 0), "1280 宽时顶部菜单应在 125 宽小地图左侧贴顶")
	...
	var message_center: Vector2 = (hud.system_message_feed.get_child(0) as Label).get_global_rect().get_center()
...
func _assert_minimap_modes(hud: CanvasLayer) -> void:
	...
	_expect(hud.top_menu.position == Vector2(824, 0), "小地图变宽时顶部菜单应重新锚定在其左侧")
	...
	_expect(hud.top_menu.position == Vector2(999, 0), "小地图收起后顶部菜单应按 125 像素宽度重新锚定")
...
func _assert_1600_layout(hud: CanvasLayer) -> void:
	_expect(hud.top_menu.position == Vector2(1319, 0), "1600 宽时顶部菜单应仍位于小地图左侧")
	_expect(hud.top_menu.size == Vector2(156, 56), "窗口变大不得缩放顶部菜单")
