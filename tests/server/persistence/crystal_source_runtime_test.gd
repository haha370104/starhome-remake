...
func _run() -> void:
	...
	config.player_state_store_path = "res://.godot/crystal-source-runtime-%d.json" % Time.get_ticks_usec()
	if not _boot():
		quit(1)
		return
	...
	player.inventory.add_reward(catalog.create("glory_equipment_newequip_shan_e010f43b10", {"instance_id": "crystal.body"}).value)
	player.inventory.add_reward(catalog.create("crystal_source_core_red_2", {"instance_id": "crystal.core", "crystal_source_cracks": 2, "bound": true}).value)
	var quote := PlayerCrystalSourceActions.growth_quote(player, catalog, "crystal.body", "quality", false)
	for cost: Dictionary in quote.value.requirements:
		var remaining := int(cost.quantity)
		while remaining > 0:
			var material: GameItem = catalog.create(cost.definition_id, {"instance_id": CrystalSourceService.new_id()}).value
	...
	var command := {"type": "grow_crystal_source", "instance_id": "crystal.body", "mode": "quality", "confirmed": true, "inventory_revision": _state().inventory_revision}
	...
	_check(server.handle_peer_player_panel_command(PEER, {"type": "inlay_crystal_source", "instance_id": "crystal.body", "material_id": "crystal.core", "mode": "inlay", "index": 0, "inventory_revision": _state().inventory_revision}).ok, "durable inlay")
	_check(combat.actors[entity_id].weapons["energy_cannon.primary"].minimum_damage == original_attack, "backpack growth inactive")
	var equip := {"type": "equip_vehicle_item", "instance_id": "crystal.body", "location": 28, "inventory_revision": _state().inventory_revision, "loadout_revision": _state().vehicle_loadout_revision}
	_check(server.handle_peer_player_panel_command(PEER, equip).ok, "stable special slot resolves")
	_check(combat.actors[entity_id].weapons["energy_cannon.primary"].minimum_damage == original_attack + 8, "actual cannon consumes core bonus")
	_check(combat.vehicle_state_for(entity_id).max_health == original_maximum + 330, "actual battle health uses body growth")
	...
	_check(combat.actors[entity_id].weapons["energy_cannon.primary"].minimum_damage == original_attack + 8 and combat.vehicle_state_for(entity_id).max_health == original_maximum + 330, "restart preserves actual effects")
	player = server.player_panel_service._mapper.to_domain(_state()).value
	_check(player.vehicle.loadout.at(28).crystal_source.quality == 1 and player.vehicle.loadout.at(28).crystal_source.slots[0].cracks == 2 and player.vehicle.loadout.at(28).bound, "restart exact growth cracks and binding")
	_check(server.handle_peer_player_panel_command(PEER, {"type": "unequip_vehicle_item", "location": 28, "inventory_revision": _state().inventory_revision, "loadout_revision": _state().vehicle_loadout_revision}).ok, "unequip special")
	...
	print("Crystal source runtime: %d checks, %d failures" % [checks, failures.size()])
