...
func _run() -> void:
	...
	config.player_state_store_path = "res://.godot/austin-runtime-%d.json" % Time.get_ticks_usec()
	if not _boot():
		quit(1)
		return
	...
	player.inventory.add_reward(catalog.create("glory_equipment_austinglens_gh_4c25a84000", {"instance_id": "austin.body"}).value)
	player.inventory.add_reward(catalog.create("austin_totem_rune", {"instance_id": "austin.rune", "bound": true}).value)
	player.inventory.add_reward(catalog.create("austin_dispel_stone", {"instance_id": "austin.stones", "quantity": 5}).value)
	var quote := PlayerAustinActions.quote(player, catalog, "austin.body", "color", 0, "")
	...
	var command := {"type": "change_austin_glens", "instance_id": "austin.body", "mode": "color", "confirmed": true, "inventory_revision": _state().inventory_revision}
	...
	_check(server.handle_peer_player_panel_command(PEER, {"type": "change_austin_glens", "instance_id": "austin.body", "mode": "unlock", "confirmed": true, "index": 0, "inventory_revision": _state().inventory_revision}).ok, "durable opening")
	_check(server.handle_peer_player_panel_command(PEER, {"type": "change_austin_glens", "instance_id": "austin.body", "material_id": "austin.rune", "mode": "inlay", "confirmed": true, "index": 0, "inventory_revision": _state().inventory_revision}).ok, "durable permanent rune")
	_check(combat.actors[entity_id].weapons["energy_cannon.primary"].minimum_damage == original_attack, "backpack growth inactive")
	var equip := {"type": "equip_vehicle_item", "instance_id": "austin.body", "location": 24, "inventory_revision": _state().inventory_revision, "loadout_revision": _state().vehicle_loadout_revision}
	_check(server.handle_peer_player_panel_command(PEER, equip).ok, "stable special slot resolves")
	_check(combat.actors[entity_id].weapons["energy_cannon.primary"].minimum_damage == original_attack + 2, "actual cannon includes Austin base tracks")
	_check(combat.vehicle_state_for(entity_id).max_health == original_maximum + 110, "actual battle health uses body growth")
	...
	_check(combat.actors[entity_id].weapons["energy_cannon.primary"].minimum_damage == original_attack + 2 and combat.vehicle_state_for(entity_id).max_health == original_maximum + 110, "restart preserves actual effects")
	player = server.player_panel_service._mapper.to_domain(_state()).value
	_check(player.vehicle.loadout.at(24).austin_glens.color == 1 and player.vehicle.loadout.at(24).austin_glens.slots[0].rune_id == "austin_totem_rune" and player.vehicle.loadout.at(24).bound, "restart exact color permanent rune and binding")
	_check(server.handle_peer_player_panel_command(PEER, {"type": "unequip_vehicle_item", "location": 24, "inventory_revision": _state().inventory_revision, "loadout_revision": _state().vehicle_loadout_revision}).ok, "unequip special")
	...
	print("Austin runtime: %d checks, %d failures" % [checks, failures.size()])
