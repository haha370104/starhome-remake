...
func _initialize() -> void:
	...
	for id: String in catalog.sama_rules.profiles: _equipment(id)
	print("Sama integration: %d checks, %d failures" % [checks, failures.size()])
...
func _equipment(id: String) -> void:
	...
	var profile := catalog.sama_rules.profiles[id]
	var growth := SamaGrowth.new()
	growth.color = 3
	growth.growth = 10
	growth.quality = 10
	var item: VehicleEquipment = catalog.create(id, {"instance_id": "sama.body", "sama": growth.to_dictionary()}).value
	...
	_check(not catalog.create("iron_piece", {"sama": growth.to_dictionary()}).is_ok, "foreign material growth rejected")
	_check(not catalog.create("recruit_tank", {"sama": growth.to_dictionary()}).is_ok, "foreign vehicle growth rejected")
	player.vehicle.loadout.equip(catalog.create("starter_missile", {"instance_id": "sama.missile"}).value, 13, player.vehicle.loadout.revision)
	...
	for attribute: String in SamaRules.ATTRIBUTES:
	...
	_check(copied_item.sama.to_dictionary() == growth.to_dictionary() and copied_item.sama_rules == catalog.sama_rules, "runtime facts and shared rules")
	copied_item.sama.growth = 1
	_check(item.sama.growth == 10, "runtime growth not aliased")
	var restored := _roundtrip(player)
	_check(restored.vehicle.loadout.at(profile.location).sama.to_dictionary() == growth.to_dictionary(), "equipped JSON")
	...
	_check(client.vehicle.loadout.at(profile.location).sama.to_dictionary() == growth.to_dictionary(), "client projected growth")
	...
	_check(restored.warehouse.find(item.instance_id).sama.to_dictionary() == growth.to_dictionary(), "warehouse JSON")
	...
	_check(restored.vehicle.loadout.at(profile.location).sama.to_dictionary() == growth.to_dictionary(), "preset keeps full growth")
	_check(item.maintenance_profile != null and item.maintenance_profile.regular_allowed, "repair chain enabled")
	var health_before := item.durability
	item.record_use("damage", 100)
	_check(item.durability == health_before - 1, "special equipment wears from authoritative damage")
	item.durability = 0
	for attribute: String in SamaRules.ATTRIBUTES: _check(item.special_bonus(attribute) == 0, "damaged passive suppressed")
