...
func calculate_stats(
	...
	var final_defense := maxi(0, roundi(clothing_bonuses.apply_value("defense", defense + loadout.socket_bonus("defense") + loadout.generator_bonus("defense"))) + food_status.bonus(17))
...
func reconcile_loadout_state(preserve_resource_ratios := true) -> bool:
	...
	max_health = maxi(1, roundi(clothing_bonuses.apply_value("max_health", chassis.base_max_health + achievement_bonuses.max_health + loadout.attachment_bonus("max_health") + loadout.socket_bonus("max_health") + loadout.generator_bonus("max_health"))) + food_status.bonus(16))
...
func enhanced_attack(attribute: String, base: int, title_bonus: int, food_kind: int) -> int:
	if base <= 0:
		return 0
	return maxi(1, roundi(clothing_bonuses.apply_value(attribute, base + title_bonus + loadout.attachment_bonus(attribute) + loadout.socket_bonus(attribute) + loadout.generator_bonus(attribute))) + food_status.bonus(food_kind))
