...
func _valid_projectile(projectile_value: Variant) -> bool:
	if not projectile_value is Dictionary:
		return false
	var projectile: Dictionary = projectile_value
	var offset_value: Variant = projectile.get("offset", [])
	return (
		not String(projectile.get("resource", "")).is_empty()
		and int(projectile.get("frames", 0)) > 0
		and float(projectile.get("fps", 0.0)) > 0.0
