...
> 历史纵切记录：下文“当前范围”指最初大厅→城区→D04 接线阶段。
> 全量导入后 G08 已有运行资源，最新覆盖见[运行内容](./runtime_content.md)；
> 原始拓扑与来源证据仍保留供核对。
...
2026-09-01 的运行时扩展又从这 26 条中提升了八个已有荣耀地图包的服务入口：裁缝店、
食品店、娱乐大厅、提炼中心 1、杂货店、交易中心、植物园和武器店。城区现有 13 条可用边，
其余 18 条继续延后；美容室、花房、提炼中心 2/3 等目标在当前荣耀运行索引中没有可装载
定义，不能用名称相近的房间代替。武器店与裁缝店的返程边有源 FCC 证据；另外六张室内图
只确认了南侧门洞和导航，因此返程入口统一放在门洞前的可走格，并以
`navigation_derived` 写入 `runtime_transition_overrides_v1.json`，没有伪称原服务端出生坐标。

当前运行时目录已经按这条真实链路及上述服务设施登记十一张地图，City 与 D04 的 MapDefinition、导航、
