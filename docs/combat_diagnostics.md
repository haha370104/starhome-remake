...
## 2026-09-18：异步战斗日志与开炮帧耗时

保留调试构建默认开启日志、现有字段、JSONL结构与64MiB单文件限制。
主线程冻结记录的时间及字段后入队，后台低优先级线程每批约50ms合并写入，保持事件顺序，
共用文件句柄并按批刷新；目录创建、文件打开、JSON序列化与落盘不占用战斗帧。
日志线程在场景加载期启动，进程生命周期节点在正常退出时排空并关闭，避免尾部记录被遗漏。
人工即时读日志和测试可用 `CombatTraceLogger.flush()` 等待；普通战斗不调用这个等待接口。
队列上限8192条；异常持续拥塞会拒绝新增记录并提示，不阻塞游戏线程或无限累积内存。

当前C07存档隔离副本，RTX4060/OpenGL，1280×720、60FPS上限，每段4秒，每450ms点击一次。
日志开启的两轮均成功发射9发，修复前后每轮各108条记录，各阶段数量相同：

| 指标 | 同步日志与3秒同步存档 | 异步日志与60秒后台检查点 |
| --- | ---: | ---: |
| 第一轮开炮请求最大耗时 | 49.223ms | 0.686ms |
| 第二轮开炮请求最大耗时 | 11.997ms | 0.583ms |
| 第一轮最慢画面帧 | 107.972ms | 31.558ms |
| 第二轮最慢画面帧 | 73.547ms | 29.252ms |

关闭日志作对照时，请求最大耗时为0.420/0.413ms；说明同步日志是可复现的额外开销。
画面指标同时受存档策略调整影响，不能把全部帧改善都归因于日志；该样本是定点发射至射程末端，
不作为大量怪物、集中击杀或首次加载所有特效的性能保证。完整记录见
...
日志线程启动时，终端会打印“战斗诊断日志：<绝对路径>”。单机调试模式的客户端和进程内权威
