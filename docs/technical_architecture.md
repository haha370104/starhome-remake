...
客户端会话只依赖 `ClientTransportEndpoint`：联机使用 `NetworkTransportEndpoint` 将命令
编码为 ENet RPC；编辑器与单机调试使用 `InProcessAuthoritativeTransport`，以异步消息队列
模拟传输边界并在同一进程中运行真正的 `AuthoritativeServer`。两种实现都调用服务器唯一的
`dispatch_transport_command()`，也消费同一种可靠消息和按 peer 快照。进程内模式不得另建
战斗、面板、掉落、技能、寻路或存档权威；运行差异只允许存在于数据如何跨越传输边界。
...
`acknowledged_input_sequence`。这里的确认序号只表示服务器已经接收并开始执行目的地命令，
不表示服务器已走到客户端当前表现位置。`LocalMovementPredictor` 因此会在本地路线与服务器
路线任一方仍在执行时保留连续的本地表现，不能用每个 10 Hz 快照把角色拉回权威旧坐标；双方
均抵达终点后才收敛最终脚点，服务器提前停在不同终点时仍会触发校正。新目的地尚在可靠通道
传输途中时，同样忽略描述旧路线的快照，从而支持连续点地和走位。远端角色仍在快照间插值。
当前协议仍处于首个未发布版本，故不提供旧字段兼容别名；客户端和契约层会
...
2026-09-17 实体快照增加可选的 `movement_speed`（0..MAX_SPEED）：表示装备计算后的可用
移动速度，静止时仍下发；已有 `speed` 继续表示瞬时速度，静止为零。旧八字段快照可继续
读取与往返序列化，缺失可用速度不会覆盖已有预测速度。服务器始终提供新字段，经
`EntitySnapshot` 校验、预测器的 tick 门禁和表现信号传给 `LocalPlayerController`，包括
初次进入地图与行进中换装。旧 tick 不得回退移速，预测器切图重置会清除旧速度。
客户端每帧与权威实体一样消费完整移动距离，跨过拐点后继续使用剩余距离；不再以固定
203 px/s 播放所有战车，也不因渲染帧率和短路径段累积落后。
...
- 客户端启动后立即显示不依赖具体地图素材的全屏加载画面。默认大厅只作为资源装配占位，
  在 `session_opened/map_joined` 恢复持久化角色地图并完成对应 bundle 原子提交前不得露出；
  连接失败时加载画面保留并展示错误，不回退显示错误地图。
...
都不会进入结算。编辑器离线模式通过进程内传输启动同一个 `AuthoritativeServer`，不再拥有
客户端战斗桥或第二套模块组装流程。

战斗快照中的 `recent_events` 是异构领域事件窗口：只有递增 `event_id` 和 `event_type` 是
所有事件共有的契约，伤害目标、扣血量、自维修角色、掉落实体等字段由各事件类型分别验证。
客户端不得用某一种伤害事件的字段要求校验整个事件窗口，更不得因为单条表现事件缺少不相干
字段而丢弃同一快照中的怪物位置、动作和生命值；否则会把正常的 10 Hz 权威更新表现为怪物
静止及伤害长时间积压后一次跳变。

客户端预测弹体与服务端权威弹体必须保留可观测但不参与玩法裁决的关联链：本地
`visual_shot_id` 通过 `input_sequence` 关联服务器 `shot_id`。调试构建按
[战斗命中差异诊断](./combat_diagnostics.md) 同时记录客户端逐帧扫掠几何、服务端发射 tick 的
权威定靶几何、命令拒绝和最终失效原因；诊断字段不得反向成为命中或伤害输入。

怪物表中的已恢复生命、基础攻击与未知字段保持证据原义；为可玩纵切增加的 AI 速度、攻击
距离、游荡半径和基准 5 秒空闲游走间隔均显式标记 `reconstructed_default`；游荡实际等待按
[个体计时规则](map_residency_and_performance.md#2026-09-17-怪物游荡错峰) 分散。尤其奥姆幼虫和毒胶的已恢复基础攻击为
...
障碍物推进或由逐点吸附造成边缘抽动。正式联机与进程内模式运行同一个地图实例及
`MonsterRoutePlanner`，传输实现没有权限替换怪物移动规则。
...
进程内模式也经过同一个服务器“战斗预检 → 正式背包规则入账 → 地面实体提交删除”事务。原客户端
