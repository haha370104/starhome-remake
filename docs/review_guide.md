...
见 `tests/ui/runtime/client_preferences_test.gd`、`client_settings_panel_test.gd`、`graceful_exit_client_test.gd`。
...
撒玛按[独立台账](sama_equipment.md)审查成长、材料供给与贯穿/脉冲/核变结算。

中枢按[独立台账](central_controller.md)区分角色永久芯片、装备阶数与PVP延期能力；核查槽位兼容与真实供给。
