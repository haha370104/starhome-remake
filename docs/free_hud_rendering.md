...
2026-09-15 按用户澄清恢复原版图标：八个保留入口排成两行四列，工具栏156×56，
按钮保留原版29/31×27像素正常、悬停、按下三态，名称仅在悬停提示中出现。
...
【当前精简布局】第一行队伍、返回基地、自维修、召唤守卫；第二行智脑、中枢控制器、佣兵任务、人物历练。
旧背景烘焙了被删除的图标，因此当前只复用独立按钮三态及收展图像，底板绘制为紧凑蓝色底色，不显示整张旧背景。
系统消息、帮助等旧图像仍作为来源记录保留，但不创建入口。增量恢复工具 `tools/import_retained_top_menu_icons.py`
只导出保留入口并合并来源审计，不重建其他HUD区域；全量导入表也已登记新增的五个原图标。
