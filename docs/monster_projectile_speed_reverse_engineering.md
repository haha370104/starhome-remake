...
| 毒胶及其同类变种 | `NpcSlm1/2/3` | `rotbullet` | 无 | 8 帧定向喷射，不使用空间线速度 |

复刻版已经把奥姆虫和奥姆幼虫的共享客户端/服务端速度从错误的 `1000 px/s` 调整为 `416.666667 px/s`。毒胶当前仍保留一项明确标注为临时值的服务端到达时序；等其 8 帧喷射模型实现后，应删除这个伪弹速，而不是继续套直线弹公式。
...
`MissileBullet` 初始参数 600，有效速度约 **250 px/s**；脚本在若干次追踪更新后把参数提升到 2000，对应约 **833.333 px/s**。`GuardMissileBullet` 从 1000 起步，之后同样可提升到 2000，约为 **416.667 → 833.333 px/s**。
