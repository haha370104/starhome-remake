...
- 精英使用独立附加组，不计入普通怪五种上限；变异和BOSS另有独立附加组。
...
20种共享名额且按等权补缺。
精英人数与计时独立于普通怪，普通与精英上限合计230；新增变异和BOSS另计。死亡精英不逐只原地复活、不被一分钟普通补量补回。
...
27种变异只投放在含对应本体的地图：每个本体组额外2只，每600秒补缺。
10种BOSS分配至24张10级图，每图固定2种各1只，每1800秒补缺；不挤占普通或精英名额。
变异生命为本体15倍，攻击、防御为本体3倍；原目录未确认的防御仍按运行时0处理，未伪造原版数值。
变异毒胶不攻击，其余变异和BOSS不主动索敌，受击后还击。

保留原材料掉落，额外独立判定人物强化材料。变异按生命五档设置每次期望：
词条石0.18/0.30/0.45/0.65/0.90、宝石0.30/0.55/0.80/1.10/1.50；
零攻击种（变异毒胶和变异奥姆幼虫）减半。词条最高明亮，宝石最高7级。
BOSS按生命强度连续插值：词条期望1.5～3.5、宝石3～8；
词条仅平凡～完美，宝石仅5～10级。新材料不乘精英20倍，仍经过账号掉落切面。

完整本体对应、品质权重与逐项概率在[强化怪物策略](../data/gameplay/enhancement_monsters_v1.json)，
由 `tools/build_clothing_enhancement_content.py` 生成；不手改产物。
专项 `tests/server/combat/enhancement_population_test.gd` 22432项通过，
原精英种群8333项回归通过，覆盖独立配额、刷新边界、掉落范围和反击行为。
