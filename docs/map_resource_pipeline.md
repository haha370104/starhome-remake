...
最初地图纵切与出生点证据见[阶段 2 图谱种子](./stage2_map_graph_seed.md)（历史范围）；
当前全量可装载目录见[运行内容](./runtime_content.md)，服务端生命周期见[地图驻留](./map_residency_and_performance.md)。

本文记录 FancyBoxII 老客户端地图资源的来源、离线解析方式，以及
`starhome_remake` 在 Godot 中的消费约定。结论来自客户端文件、引擎 DLL
导出的 PKH 解压函数和实际数据交叉验证；地图解析本身不抓游戏内存，也不依赖登录器或服务器。
前置 FCC 解密参数首次通过客户端内存读取确认，不能将两件事混淆，见[FCC 恢复流程](./fcc_resource_recovery.md)。

## 1. 现在的大厅地图来自哪里

当前基地大厅已切换为荣耀版 `RoomSvr1`；显示文案使用龙之城/地图实际城市名，
`yian_harbor_*` 仍是已有稳定运行 ID，不在文案修正时顺手迁移存档键。该大厅使用
...
D04 的十二条有效出口按旧样式编号 1 至 8 对应荣耀版
`pic3/interface/sportimg/as1-as8.ale` 八方向素材。旧客户端 `transport.fcc` 对普通传送器声明
`playdelay=100`，并由 ALE 自身循环播放；导入器因此严格按各自 `frames.json` 的顺序消费全部
9 或 10 帧，不再从单帧人为生成明暗脉冲。运行时资源按
`east/south_east/south/south_west/west/north_west/north/north_east` 业务方向共享，旧目录和
文件名只保留在 `import_metadata.json` 来源审计中。

地图表现构建器必须把 `transitions.json` 视为传送图标的唯一所有者。若
`scene_objects.json` 中还存在与传送记录“源 ALE 路径＋图标锚点”完全相同的 `AddImgEx`，
该对象只代表旧客户端脚本中的动画摆放，不得再以首帧写入静态语义 atlas。有效传送点由
运行时注册创建 `AnimatedSprite2D`；`disabled_legacy` 对应的旧出口也从静态场景排除，但不
创建运行时对象。这样既避免静态首帧遮住动画，也避免已经禁用的出口留下误导性图标。
这条规则同时适用于语义分层地图和全量地图内容包：后者虽然仍输出单张 `floor.png`，构建器
也必须先以去除传送摆放后的场景恢复其像素，再写入压平地板，不能直接复制解析阶段的
`composite.png`。

2026-09-17 排查 C04→C05 时发现，既有运行时动画和排除规则已正确，但 C03、C04、C05、
D03、D05 的独立目录仍是旧构建产物。重新执行 `import_glory_map_presentation.import_map`
重建这五张布里野外地图；C03/C04/D03/D05 分别剔除 3/5/5/8 个静态传送摆放，共 21 个。
C05 原场景没有箭头摆放，只更新构建审计；原始地板、小地图与导航未改变。普通场景仍按
语义层排序，传送点仅由共享 `MapTransitionView` 创建多帧动画，z=-1 位于战车之下。

新增 `tests/integration/field_transition_presentation_test.gd` 并纳入客户端门禁，遍历基础
目录与内容包中已发布、具有出口的 91 张野外地图和 591 个出口，检查排除标记、方向资源、
循环帧数与层级，并实际提交上述五张地图验证动画推进。明确标记未完成素材导入的 G08
空壳不计入可玩表现审计。内容包中三张地图记录的 unresolved 源箭头在原始解包时已缺失、
从未烘焙到 composite，不能将其误报为遗留静态图；该计数必须属于已排除的摆放集合。

本轮无头和 OpenGL 实际渲染各 1986 项通过。加 `-- --capture` 可在 `.godot/` 输出
`c04-portal-frame-0.png`、`c04-portal-frame-5.png`、`c04-portal-vehicle.png`；已目视确认
C04→C05 不同帧的箭头发光变化，以及战车覆盖箭头。以后修改构建器须同步重建受影响产物，
不能仅修动画组件或仅检查 D04。
...
复刻运行层不会直接改写装在只读内容包里的 MapDefinition。若目标室内地图已经可装载、
源城区出口有 FCC 证据，但旧服务器决定的返程出生点缺失，可在
`data/maps/runtime_transition_overrides_v1.json` 增加一条显式审计覆盖。覆盖只允许补充
结构化 `transitions`，必须记录来源地图、门洞判断和 `navigation_derived` 证据，并由测试
加载真实导航验证 `approach_point` 可走；它不是用来覆盖原始出口方向或猜造不存在地图的
通用补丁。食品店、娱乐大厅、提炼中心 1、杂货店、交易中心和植物园是早期补齐范围；
后续兵工厂返程及服务室内扩展以当前 overrides、城区定义和独立地图定义为准。
宇航中心已使用 `dragon_city_space_center.json`，不能恢复为此前错误的地图复用。

### 缺失场景素材人工审计

地图保持缺失位置为空白，不通过相似文件名猜图。完成荣耀本地/缓存及精确官网补抓后，可批量生成
