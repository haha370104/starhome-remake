...
右键“使用”会根据服务端实时储备能量，对当前选中堆叠进行一次批量消耗：

1. 能量已满则返回 `items.energy_full`，不扣物品、不推进背包版本。
2. 可容纳一包以上时，使用 `floor(剩余容量 / 单包恢复量)` 包，最多不超过当前堆叠数量。
3. 剩余容量小于一包时消耗一包，恢复至上限，溢出部分丢弃。
4. 其他堆叠不自动参与扣除，食品仍每次消耗一份；锁定、人物死亡、战车损毁继续拒绝使用。

例如1000能量包、战车3500/10000、当前堆叠至少七包：第一次扣六包得到9500，
第二次扣一包得到10000，第三次报满能量错误。有三包则第一次全部使用，得到6500并移除空堆叠。
该行为适用于低级、中级及兼容旧定义的能量包，单包数值仍来自原目录。

`PlayerVehicle.energy_pack_use_count` 持有容量与取整规则，`PlayerConsumableActions` 在玩家聚合内
一次扣数量、一次补能并推进一次背包版本。服务端先捕获地图实时能源，再生成候选并提交完整存档；
客户端传入的数量、当前能源、恢复值都不参与计算。操作补充的是储备能量，不直接补工作能量，
也不会重置射击冷却。此批量规则来自用户要求，不冒充已确认的原服实现。

领域和正式服务器专项覆盖上述三次使用、整除、浮点余量、包数不足、移除空堆叠、多个堆叠隔离、
锁定/死亡拒绝、重复版本拒绝，以及存档与战斗运行态一致；测试存档全部位于 `.godot/` 隔离目录。
