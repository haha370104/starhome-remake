...
## 2026-09-17 用户确认跨版本恢复后的状态

上节“6款缺失”的结论是恢复前状态，现已按用户确认用免费版补齐这6款的世界/背包/面板素材。
纳米前护甲和帝王能量炮的差异面板也已明确选择免费版。详细来源、范围和全量候选表见
[缺失素材恢复](cross_release_asset_recovery.md)。当前40款底盘均有世界动画；6款恢复条目仍沿用原装备身份和数值。
装备专项19277项通过，实际OpenGL渲染19325项通过；背包/面板原生尺寸与原点加入现有真实面板回归。

用户随后授权全部候选接入：累计188个原引用、180个物品定义、430个展示位，武器弹体可用绑定从44增至50。
物品覆盖仅更新表现，装备身份、槽位、属性与八方向契约保持不变。新的素材索引回归和实际表现回归已纳入客户端门禁。
