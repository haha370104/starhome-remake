...
- 预览锚点为 `(74,231)`；ALE 原点参与对齐，导入时不得只按图片左上角摆放。
- 预览背景为 `pic3/interface/char/manbackground.jpg`。

### 4.2 人像合成

【客户端确认】衣服不是预烘焙到人物图里。每件已穿服装使用自己的 `m_sdlgsrc`，放在约 `(91,274)`，并以 `m_nLayer` 决定 Z 顺序。不同性别可能切换衣服子图。人物变形时会隐藏普通服装层。
...
旧源码还出现 19..23、28..31 等后期活动/补丁槽。首批不必显示，但数据和协议必须允许未知 location，不能把枚举写死为 0..18 后直接丢弃存档数据。

`m_nEquipKind2` 是装备大类提示：0 战车、1 炮、2 引擎、3 维修、4 挖掘、5 护甲、6 隐身、7 雷达、8 导弹、9 火箭。它适合做“该物品可投到哪些槽”的校验，不能替代 location。

### 6.3 中央预览的合成

【客户端确认】每件装备的 `EquipInDlg()` 会指定自己的对话框素材、锚点和 Z 顺序。例如车体大致锚在 `(110,60)`，主武器常锚在 `(110,65)` 或 `(100,70)`；不同车种还会让主武器位于车体前面或后面。护甲、推进器和扩展设备各有自己的硬编码位置。
...
`game_window_manager` 只负责打开/关闭、置顶、窗口边界和冻结状态；不持有人物、背包或战车业务数据。三个 presenter 订阅客户端状态仓库，不能互相直接改节点。
